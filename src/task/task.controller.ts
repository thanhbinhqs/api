...
export class TaskController {
    ...
    @UseGuards(PermissionGuard)
    @HasPermission(Permission.TASK_CREATE)
    async createRichTask(@Body() createRichTaskDto: CreateRichTaskDto, @Request() req) {
        return await this.taskService.createRichTask(createRichTaskDto, req.user.id);
    }
    ...
    async getStatistics(@Request() req, @Query('userId') userId?: string) {
        ...
        const targetUserId = userId && req.user.permissions?.includes(Permission.TASK_MANAGE_ALL) 
            ? userId 
            : req.user.id;
    ...
    async attachFiles(
        ...
        return await this.taskService.attachFilesToTask(id, attachFilesDto, req.user.id);
    }
