...
export class NotificationIntegrationService {
  ...
  async notifyByPermission(
    ...
    try {
      ...
      this.logger.error(
        `Error notifying by permission ${permission}: ${error.message}`,
      );
    }
  }
  ...
  async notifyUserRoleChanged(
    ...
    await this.notifyAdminAndManagers(
  ...
  async notifySystemSettingsUpdated(
    settingKey: string,
    updatedBy: string,
  ): Promise<void> {
    const data: NotificationData = {
