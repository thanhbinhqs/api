...
import { InOutHistory, InOutType } from 'src/meta/entities/inout-history.entity';
...
export class JigOrderService {
    ...
    async create(createDto: CreateJigOrderDto, requesterId: string): Promise<JigOrder> {
        ...
        try {
            ...
            const order = queryRunner.manager.create(JigOrder, {
                ...
                requestedDate: createDto.requestedDate ? new Date(createDto.requestedDate) : new Date(),
            ...
            for (const detailDto of createDto.orderDetails) {
                ...
                if (!jigDetail) {
                    throw new NotFoundException(`Không tìm thấy jig detail với ID ${detailDto.jigDetailId}`);
                }

                // Kiểm tra availability
                if (jigDetail.status === JigStatus.SCRAP) {
                    throw new BadRequestException(`Jig detail ${jigDetail.code} đã bị hủy, không thể order`);
                }
    ...
    async findAll(query: JigOrderQueryDto): Promise<PaginatedResult<JigOrder>> {
        const queryBuilder = this.jigOrderRepository.createQueryBuilder('order')
        ...
        if (query.requesterId) {
            queryBuilder.andWhere('order.requester.id = :requesterId', { requesterId: query.requesterId });
        }

        if (query.approverId) {
            queryBuilder.andWhere('order.approver.id = :approverId', { approverId: query.approverId });
        }

        if (query.fromDate) {
            queryBuilder.andWhere('order.createdAt >= :fromDate', { fromDate: query.fromDate });
        }

        if (query.toDate) {
            queryBuilder.andWhere('order.createdAt <= :toDate', { toDate: query.toDate });
        }
    ...
    async update(id: string, updateDto: UpdateJigOrderDto, userId: string): Promise<JigOrder> {
        const order = await this.findOne(id);

        // Chỉ cho phép cập nhật khi ở trạng thái DRAFT hoặc là người tạo
        if (order.status !== JigOrderStatus.DRAFT && order.requester.id !== userId) {
        ...
        try {
            ...
            Object.assign(order, {
                ...
                requestedDate: updateDto.requestedDate ? new Date(updateDto.requestedDate) : order.requestedDate,
                requiredDate: updateDto.requiredDate ? new Date(updateDto.requiredDate) : order.requiredDate,
            ...
            if (updateDto.orderDetails) {
                ...
                await queryRunner.manager.delete(JigOrderDetail, { order: { id: order.id } });
    ...
    async submit(id: string, userId: string): Promise<JigOrder> {
        ...
        if (order.status !== JigOrderStatus.DRAFT) {
            throw new BadRequestException('Chỉ có thể submit đơn hàng ở trạng thái DRAFT');
        }
        ...
        const approvalPriority = this.mapOrderPriorityToApprovalPriority(order.priority);
    ...
    async approve(id: string, approveDto: ApproveJigOrderDto, approverId: string): Promise<JigOrder> {
        const order = await this.findOne(id);

        if (order.status !== JigOrderStatus.SUBMITTED) {
            throw new BadRequestException('Chỉ có thể phê duyệt đơn hàng ở trạng thái SUBMITTED');
        }
    ...
    async reject(id: string, rejectDto: RejectJigOrderDto, approverId: string): Promise<JigOrder> {
        const order = await this.findOne(id);

        if (order.status !== JigOrderStatus.SUBMITTED) {
            throw new BadRequestException('Chỉ có thể từ chối đơn hàng ở trạng thái SUBMITTED');
        }
    ...
    async prepare(id: string, prepareDto: PrepareJigOrderDto, preparerId: string): Promise<JigOrder> {
        const order = await this.findOne(id);

        if (order.status !== JigOrderStatus.APPROVED) {
            throw new BadRequestException('Chỉ có thể chuẩn bị đơn hàng đã được phê duyệt');
        }

        const preparer = await this.userRepository.findOne({ where: { id: preparerId } });
        if (!preparer) {
        ...
        try {
            ...
            for (const preparedDetail of prepareDto.preparedDetails) {
                ...
                if (!orderDetail) {
                    throw new NotFoundException(`Không tìm thấy order detail với ID ${preparedDetail.orderDetailId}`);
                }
            ...
            const allPrepared = order.orderDetails.every(detail => detail.isPrepared);
            if (allPrepared) {
    ...
    async notify(id: string, notifyDto: NotifyJigOrderDto, notifierId: string): Promise<JigOrder> {
        const order = await this.findOne(id);

        if (order.status !== JigOrderStatus.READY) {
            throw new BadRequestException('Chỉ có thể thông báo đơn hàng đã sẵn sàng');
        }
    ...
    async pickup(id: string, pickupDto: PickupJigOrderDto, pickupUserId: string): Promise<JigOrder> {
        const order = await this.findOne(id);

        if (order.status !== JigOrderStatus.NOTIFIED) {
            throw new BadRequestException('Chỉ có thể pickup đơn hàng đã được thông báo');
        }

        // Kiểm tra quyền pickup (chỉ requester hoặc người được ủy quyền)
        const actualReceiver = pickupDto.actualReceiverId ? 
            await this.userRepository.findOne({ where: { id: pickupDto.actualReceiverId } }) :
    ...
    async getMyOrders(userId: string, query: JigOrderQueryDto): Promise<PaginatedResult<JigOrder>> {
        query.requesterId = userId;
        return await this.findAll(query);
    }

    async getPendingApprovals(query: JigOrderQueryDto): Promise<PaginatedResult<JigOrder>> {
        query.status = JigOrderStatus.SUBMITTED;
        return await this.findAll(query);
    }

    async getPendingPreparations(query: JigOrderQueryDto): Promise<PaginatedResult<JigOrder>> {
        ...
        if (modifiedQuery.priority) {
            queryBuilder.andWhere('order.priority = :priority', { priority: modifiedQuery.priority });
        }

        // Sorting
        queryBuilder.orderBy(`order.${modifiedQuery.sortBy}`, modifiedQuery.sortOrder);
    ...
    private async generateOrderCode(): Promise<string> {
        ...
        let sequence = 1;
        if (lastOrder) {
            const lastSequence = parseInt(lastOrder.orderCode.substring(prefix.length));
    ...
    private mapOrderPriorityToApprovalPriority(priority: JigOrderPriority): ApprovalPriority {
        switch (priority) {
    ...
    async remove(id: string, userId: string): Promise<void> {
        ...
        if (![JigOrderStatus.DRAFT, JigOrderStatus.REJECTED].includes(order.status) || 
