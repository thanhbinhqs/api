...
export class JigDrawingService {
  ...
  async create(createJigDrawingDto: CreateJigDrawingDto): Promise<JigDrawing> {
    ...
    if (!jig) {
      throw new NotFoundException(
        `Jig với ID ${createJigDrawingDto.jigId} không tồn tại`,
      );
    }
  ...
  async findAll(filterDto: JigDrawingFilterDto): Promise<{
    ...
    const queryBuilder = this.jigDrawingRepository
      .createQueryBuilder('jigDrawing')
      .leftJoinAndSelect('jigDrawing.jig', 'jig');

    // Apply filters
    if (filters.name) {
      queryBuilder.andWhere('jigDrawing.name ILIKE :name', {
        name: `%${filters.name}%`,
      });
    }
    ...
    if (filters.drawingType) {
      queryBuilder.andWhere('jigDrawing.drawingType = :drawingType', {
        drawingType: filters.drawingType,
      });
    }

    if (filters.fileFormat) {
      queryBuilder.andWhere('jigDrawing.fileFormat = :fileFormat', {
        fileFormat: filters.fileFormat,
      });
    }

    if (filters.status) {
      queryBuilder.andWhere('jigDrawing.status = :status', {
        status: filters.status,
      });
    }

    if (filters.drawingVersion) {
      queryBuilder.andWhere('jigDrawing.drawingVersion = :drawingVersion', {
        drawingVersion: filters.drawingVersion,
      });
    }

    if (filters.drawingNumber) {
      queryBuilder.andWhere('jigDrawing.drawingNumber ILIKE :drawingNumber', {
        drawingNumber: `%${filters.drawingNumber}%`,
      });
    }

    if (filters.revision) {
      queryBuilder.andWhere('jigDrawing.revision = :revision', {
    ...
    if (filters.createdFrom && filters.createdTo) {
      queryBuilder.andWhere(
        'jigDrawing.createdAt BETWEEN :createdFrom AND :createdTo',
        {
      ...
      queryBuilder.andWhere('jigDrawing.createdAt >= :createdFrom', {
        createdFrom: filters.createdFrom,
      });
    } else if (filters.createdTo) {
      queryBuilder.andWhere('jigDrawing.createdAt <= :createdTo', {
        createdTo: filters.createdTo,
      });
    }
  ...
  async update(
    ...
    if (
      updateJigDrawingDto.filePath &&
      updateJigDrawingDto.filePath !== jigDrawing.filePath
    ) {
  ...
  async getLatestVersion(
    ...
    if (drawingNumber) {
      queryBuilder.andWhere('jigDrawing.drawingNumber = :drawingNumber', {
        drawingNumber,
      });
    }
