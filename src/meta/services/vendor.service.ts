...
export class VendorService {
    ...
    async findAllVendors(filterDto?: VendorFilterDto): Promise<PaginatedResult<Vendor>> {
        const {
            page = 1,
            limit = 10,
            sortBy = 'createdAt',
        ...
        if (description) {
            queryBuilder.andWhere('vendor.description ILIKE :description', { description: `%${description}%` });
        }
    ...
    private async checkUniqueness(name: string, code: string, excludeId?: string): Promise<void> {
        ...
        queryBuilder.where('(vendor.name = :name OR vendor.code = :code)', { name, code });
