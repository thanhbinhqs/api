...
export class LineService {
    ...
    async findAllLines(filterDto?: LineFilterDto): Promise<PaginatedResult<Line>> {
        const {
            page = 1,
            limit = 10,
            sortBy = 'createdAt',
        ...
        if (description) {
            queryBuilder.andWhere('line.description ILIKE :description', { description: `%${description}%` });
        }
    ...
    private async checkUniqueness(name: string, slug: string, excludeId?: string): Promise<void> {
        ...
        queryBuilder.where('(line.name = :name OR line.slug = :slug)', { name, slug });
