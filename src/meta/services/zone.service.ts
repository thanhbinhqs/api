...
export class ZoneService {
  ...
  async findAllZones(
    ...
    if (description) {
      queryBuilder.andWhere('zone.description ILIKE :description', {
        description: `%${description}%`,
      });
    }

    if (parentZoneId) {
      queryBuilder.andWhere('zone.parentZoneId = :parentZoneId', {
        parentZoneId,
      });
    }
  ...
  private async checkUniqueness(
    ...
    queryBuilder.where('(zone.name = :name OR zone.slug = :slug)', {
      name,
      slug,
    });
