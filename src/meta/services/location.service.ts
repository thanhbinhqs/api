...
export class LocationService {
    ...
    async findAllLocations(filterDto?: LocationFilterDto): Promise<PaginatedResult<Location>> {
        const {
            page = 1,
            limit = 10,
            sortBy = 'createdAt',
        ...
        if (description) {
            queryBuilder.andWhere('location.description ILIKE :description', { description: `%${description}%` });
        }
    ...
    private async checkUniqueness(name: string, code: string, excludeId?: string): Promise<void> {
        const queryBuilder = this.locationRepository.createQueryBuilder('location');
        
        queryBuilder.where('(location.name = :name OR location.code = :code)', { name, code });
