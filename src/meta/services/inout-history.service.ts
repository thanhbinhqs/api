...
export class InOutHistoryService {
    ...
    async createInOutHistory(createInOutHistoryDto: CreateInOutHistoryDto): Promise<InOutHistory> {
        const inOutHistory = this.inOutHistoryRepository.create(createInOutHistoryDto);
        return this.inOutHistoryRepository.save(inOutHistory);
    }

    async findAllInOutHistories(filterDto?: InOutHistoryFilterDto): Promise<PaginatedResult<InOutHistory>> {
        const {
            page = 1,
            limit = 10,
            sortBy = 'createdAt',
        ...
        if (minQuantity !== undefined) {
            queryBuilder.andWhere('inOutHistory.quantity >= :minQuantity', { minQuantity });
        }

        if (maxQuantity !== undefined) {
            queryBuilder.andWhere('inOutHistory.quantity <= :maxQuantity', { maxQuantity });
        }

        if (startDate) {
            queryBuilder.andWhere('DATE(inOutHistory.createdAt) >= :startDate', { startDate });
        }

        if (endDate) {
            queryBuilder.andWhere('DATE(inOutHistory.createdAt) <= :endDate', { endDate });
        }
        ...
        if (description) {
            queryBuilder.andWhere('inOutHistory.description ILIKE :description', { description: `%${description}%` });
        }
    ...
    async updateInOutHistory(id: string, updateInOutHistoryDto: UpdateInOutHistoryDto): Promise<InOutHistory> {
