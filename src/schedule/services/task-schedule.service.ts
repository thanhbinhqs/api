...
import { TaskType, TaskStatus, TaskPriority, AssigneeType } from '../../task/entities/task.entity';
...
export class TaskScheduleService {
    ...
    async updateOverdueTasks(): Promise<void> {
        ...
        try {
            ...
            const overdueTasks = await this.taskRepository.find({
                ...
                relations: ['assignedUsers', 'assignedRoles', 'taskCreatedBy', 'executedBy']
            });
    ...
    async notifyUpcomingTasks(): Promise<void> {
        ...
        try {
            ...
            this.logger.log(`Đã gửi thông báo cho ${upcomingTasks.length} task sắp đến hạn`);
        } catch (error) {
    ...
    private async createMaintenanceTaskForJig(jig: Jig): Promise<void> {
        ...
        maintenanceDate.setDate(maintenanceDate.getDate() + jig.maintenanceInterval);
        ...
        const maintenanceTask = this.taskRepository.create({
            ...
            scheduledEndDate: new Date(maintenanceDate.getTime() + 4 * 60 * 60 * 1000), // 4 giờ
    ...
    private async createMaintenanceTasksForJigDetails(): Promise<void> {
        ...
        for (const jigDetail of jigDetails) {
            ...
            const daysSinceLastMaintenance = jigDetail.lastMaintenanceDate 
                ? Math.floor((Date.now() - jigDetail.lastMaintenanceDate.getTime()) / (1000 * 60 * 60 * 24))
            ...
            this.logger.log(`Đã tạo maintenance task cho jig detail ${jigDetail.code}`);
        }
    }
    ...
    private async sendOverdueNotification(task: Task): Promise<void> {
        try {
            ...
            this.logger.error(`Lỗi khi gửi thông báo task quá hạn ${task.id}:`, error);
        }
    }
    ...
    private async sendUpcomingTaskNotification(task: Task): Promise<void> {
        try {
            ...
            this.logger.error(`Lỗi khi gửi thông báo task sắp đến hạn ${task.id}:`, error);
        }
    }
