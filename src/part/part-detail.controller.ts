...
export class PartDetailController {
  ...
  @ApiResponse({ status: 201, description: 'Part detail đã được tạo thành công' })
  ...
  @ApiOperation({ summary: 'Cập nhật thông tin part detail với version checking' })
  @ApiResponse({ status: 200, description: 'Part detail đã được cập nhật' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy part detail' })
  @ApiResponse({ status: 409, description: 'Conflict - Số serial đã tồn tại hoặc dữ liệu đã được cập nhật bởi người khác' })
  update(@Param('id') id: string, @Body() updatePartDetailDto: UpdatePartDetailDto) {
    return this.partDetailService.update(id, updatePartDetailDto);
  }

  @Patch('batch-status')
  @HasPermission(Permission.PART_DETAIL_UPDATE)
  @ApiOperation({ summary: 'Cập nhật status hàng loạt cho part details với version checking' })
  @ApiResponse({ status: 200, description: 'Status đã được cập nhật' })
  @ApiResponse({ status: 409, description: 'Conflict - Dữ liệu đã được cập nhật bởi người khác' })
  ...
  @ApiResponse({ status: 200, description: 'Danh sách part details của jig detail' })
  ...
  updateJigDetail(@Param('id') id: string, @Body('jigDetailId') jigDetailId: string | null) {
    return this.partDetailService.updateJigDetail(id, jigDetailId);
  }

  @Patch('batch/status')
  @HasPermission(Permission.PART_DETAIL_UPDATE)
  @ApiOperation({ summary: 'Cập nhật trạng thái hàng loạt cho nhiều part details' })
  @ApiResponse({ 
    status: 200, 
  ...
  setDefaultLocation(
    ...
    return this.partDetailService.setDefaultLocation(id, body.locationId, body.jigDetailId);
  }

  @Patch(':id/restore-default')
  @HasPermission(Permission.PART_DETAIL_UPDATE)
  @ApiOperation({ summary: 'Khôi phục part detail về vị trí mặc định' })
  @ApiResponse({ status: 200, description: 'Part detail đã được khôi phục về vị trí mặc định' })
